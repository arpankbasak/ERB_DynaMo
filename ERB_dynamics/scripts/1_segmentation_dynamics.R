...
res <- mclapply(1:length(fls), function(ind) {
    ...
    if(validObject(img)){
        ...
        merge_obj <- rgbImage(red = img[,,1] * cell, # Cell wall
                             green = img[,,2] * feature, # feature
                             blue = ((img[,,1]) * vac) + (img[,,3] * feature)) # Inside the cell + feature at time 0
...
#         etemp = NULL
        if(nrow(cdat) != 0){
            ...
            if(length(id_fpc) != 0){
                ...
                # Count number of feature signals detected
                cdat$nfeatures <- 0
                edat <- list()
                for(cell_idx in 1:nrow(cdat)){

                        # Segment ERbodies -- find the moment difference between the t0
                        # Check how far the moment have shifted in blue channel
        #                 cell_idx <- 13
...
#                         mask <- mask * (mask > quantile(mask, 0.9))
                    ...
                    if(length(edat) > 0){
                        ...
                        message("None cellular features")
#                         edata <- NULL
                    }
