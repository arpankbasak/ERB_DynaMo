...
imat_e <- do.call(rbind, res[idx])

rm(list = "res") # Remove list object to save RAM

message("Preparing image matrix of features...")
idx <- intersect(colnames(imat_c), colnames(imat_e))
imat <- rbind(imat_c[,idx], imat_e[,idx])
...
save(list = c("imat_e"), file = paste(data, "Obj_image_features.RData", sep = ""))
save(list = c("imat"), file = paste(data, "image_features.RData", sep = ""))
save(list = c("imat_c"), file = paste(data, "Cells_image_features.RData", sep = ""))
